 ...
 *
 * This is the M3 slice of the engine: linking rows that arrive out of order, and
 * computing booked / shipped / not-booked / net-accepted / fill rate / line state per
 * PO line, plus each delivery's totals and shortfall. The remaining M4 work - turnaround
 * dates, the deliver-anyway workflow, PO completion - builds on exactly these columns.
 *
 * The definitions themselves live on the models (PoLine::computeFillRate() and friends),
 * so this class decides WHEN to recompute, never WHAT the numbers mean.
 */
class Reconciler
{
    /**
    ...
    public function recomputePoLinesFor(Marketplace $marketplace, array $poNumbers, array $skuIds = []): int
    {
        ...
        $query->chunkById(500, function ($lines) use ($shipped, $cancellations, &$updated) {
            foreach ($lines as $line) {
                ...
                $line->has_chargeback_flag =
                    $cancellation?->resolution === CancellationResolution::DeliveredAnyway;
